...
public class NetworkTopology {
  ...
  protected int getWeight(Node reader, Node node) {
	...
	protected int getWeightByStorage(Node reader, Node node, StorageType storageType) {
		// 0 is local, 1 is same rack, 2 is off rack
		// Start off by initializing to off rack
		int weight = 2;
		if (reader != null) {
	  ...
	  /*
		if(storageType == StorageType.SSD){
			//LOG.info("baek -- storage type : SSD \n Node name : " + node.getName() + "-- set weigth 0 ");
			if(rate >= 30){
				weight = 3;
				this.speedRate = 0;
			}
  ...
  public void sortByDistance(Node reader, Node[] nodes, int activeLen) {
	...
	//baek added
	public void sortByStorage(Node reader, LocatedBlock b, int activeLen) {                      
		Node[] nodes = b.getLocations();
		// Sort weights for the nodes array 
		int[] weights = new int[activeLen];
		for (int i=0; i<activeLen; i++) {
			// weights[i] = getWeight(reader, nodes[i]);
			// baek added
			// weights[i] = getWeightByStorage(reader, nodes[i], (b.getStorageTypes())[i], ++speedRate);
			weights[i] = getWeightByStorage(reader, nodes[i], (b.getStorageTypes())[i]);
		}
		// Add weight/node pairs to a TreeMap to sort
		TreeMap<Integer, List<Node>> tree = new TreeMap<Integer, List<Node>>();
		for (int i=0; i<activeLen; i++) {
